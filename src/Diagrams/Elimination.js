...
function getAdditionalImpactedEntityIds(RevDiagramCopy, EliminatedEntityIds) {
    ...
    if (AdditionalImpactedEliminatedEntityIds.length > 0) {
        AdditionalImpactedEliminatedEntityIds = AdditionalImpactedEliminatedEntityIds.map((lvl) => {
            return lvl.reduce((acc, Id) => {
                return acc.concat(Id)
            })
        });
        AdditionalImpactedEliminatedEntityIds = AdditionalImpactedEliminatedEntityIds.reduce((acc, lvl) => {
            return acc.concat(lvl)
        });
    }
