...
export const getProgramHierarchy = (topprogram, programs, maxLvl, maxEntityChildren) => {
    ...
    let IterationArrayObject = new IterationArray(InitialPgmArrayObject, topprogram, maxEntityChildren);
    let FinalLvlPgmArrayObj = []

    // continue until there are no more previous level programs
    while (IterationArrayObject.PrevlevelPgms.length > 0 && IterationArrayObject.Level < maxLvl) {

        IterationArrayObject.Level++;
        // append programs to all level program array [initial starts with top level program]   

        IterationArrayObject = getcalledPgmsInNxtLvl(IterationArrayObject, programs);
        ...
        if (IterationArrayObject.PrevlevelPgmArrayObjects.length > 0) {
            FinalLvlPgmArrayObj = AddPgmstoLevelSoChildIdsAreCorrect(IterationArrayObject.PrevlevelPgmArrayObjects, IterationArrayObject.currentlevelPgmArrayObjects)
        }
        ...
        IterationArrayObject.PrevlevelPgmArrayObjects = IterationArrayObject.currentlevelPgmArrayObjects
...
function AddPgmstoLevelSoChildIdsAreCorrect(PrevlevelPgmArrayObjects, currentlevelPgmArrayObjects) {
    let FinalPgmArrayObj = [];


    if (currentlevelPgmArrayObjects.length > 0) {
        let RedcurrentlevelPgmArrayObjects = currentlevelPgmArrayObjects.reduce((acc, pgm) => {
            return acc.concat(pgm)
        })
        FinalPgmArrayObj = PrevlevelPgmArrayObjects.map((clter) => {
            clter = clter.map((obj) => {
                obj.childIds = AddChildrenIds(obj.entityId, RedcurrentlevelPgmArrayObjects)
                return obj;
            })
            return clter;
        })
...
function createPgmArrayObj(IterationArrayObj, pgm, programs, Branch, parentProgram) {
...
function getcalledPgmsInNxtLvl(IterationArrayObj, programs) {
    ...
    for (let PgmObj of PrevArrayObjects) {
        ...
        if (CalledPrograms.length > 0) {
            ...
            let Branch = getBranchPgm(IterationArrayObj.AlllevelPgmsDataArray, PgmObj);

            ClusterLevelPgmArrayObj = CalledPrograms.map((pgm) => {
                return createPgmArrayObj(IterationArrayObj, pgm, programs, Branch, parentProgram)
            })
