...
function CheckIfRecurringDuplicateOrPrevPlaced(NewDiagPgm, Diagram, furtherIterationObj, branchIterationObj) {
    ...
    if (branchIterationObj.Branch.includes(NewDiagPgm)) {
        Pgm = NewDiagPgm
        recurringInfo = AddRecurringEntityToDiagram(Pgm, Diagram, furtherIterationObj, branchIterationObj)
        ...
        if (furtherIterationObj.lvl + 1 > PrevPlacedMaxLvl) {
            if (!branchIterationObj.branchDuplicates.includes(NewDiagPgm)) {
                Pgm = NewDiagPgm;
                addedInfo2 = AddEntitiesToDiagram(Pgm, Diagram, furtherIterationObj, branchIterationObj)
...
function expandCalledPgms(NewDiagPgm, Diagram, furtherIterationObj, branchIterationObj) {
    if (furtherIterationObj.lvl < furtherIterationObj.maxLvl - 1) {
        ...
        if ((furtherIterationObj.PrevPlacedPgms.includes(NewDiagPgm)) || (branchIterationObj.Branch.includes(NewDiagPgm))) {

            const checkedInfo = CheckIfRecurringDuplicateOrPrevPlaced(NewDiagPgm, Diagram, furtherIterationObj, branchIterationObj)
            ...
            const AddedInfo = AddEntitiesToDiagram(NewDiagPgm, Diagram, furtherIterationObj, branchIterationObj)
            Diagram = AddedInfo[0];
        ...
        const AddedInfo = AddEntitiesToDiagramOnce(NewDiagPgm, Diagram, furtherIterationObj, branchIterationObj)
        Diagram = AddedInfo[0];
        furtherIterationObj = AddedInfo[1];
        branchIterationObj = AddedInfo[2];
    }
...
export const expandPgmHierarchy = (Diagram, lastEntityId, maxLvl, maxEntityChildren, programs) => {
    let DiagramBeforeExpansion = Diagram.slice(0);
    let furtherIterationObj = new furtherIteration(lastEntityId, maxEntityChildren, maxLvl, programs);
            ...
            while (Obj >= 0) {
                ...
                if (DiagramEnt.calledPrograms.length > 0) {
                    ...
                    for (let NewDiagPgm of DiagramEnt.calledPrograms) {
                        ...
                        const ExpandedInfo = expandCalledPgms(NewDiagPgm, Diagram, furtherIterationObj, branchIterationObj);
...
function AddRecurringEntityToDiagram(Pgm, Diagram, furtherIterationObj, branchIterationObj) {
    let NewDiagEnt = {}
    Pgm = Pgm + "rec"
    ...
    const NewDiagInfo = AddNewDiagEnt(NewDiagEnt, Diagram, furtherIterationObj, branchIterationObj)
...
function AddEntitiesToDiagramOnce(Pgm, Diagram, furtherIterationObj, branchIterationObj) {
...
function AddEntitiesToDiagram(Pgm, Diagram, furtherIterationObj, branchIterationObj) {
    // get programs array DiagramEnt.calledPrograms 
    // if DiagramEnt.calledPrograms < 20 mark as Moved_Object
    let NewDiagEnt = {}
    furtherIterationObj.PrevPlacedPgms.push(Pgm)
    ...
    if (!furtherIterationObj.AlreadyAddedInPrevIteration) {
        ...
        if (calledPgms.length > 0) {
            for (let calledPgm of calledPgms) {
                // already placed pgms 
                const expandedInfo = expandCalledPgms(calledPgm, Diagram, furtherIterationObj, branchIterationObj)
                Diagram = expandedInfo[0]
                furtherIterationObj = expandedInfo[1]
                branchIterationObj = expandedInfo[2]
...
function AddNewDiagEnt(NewDiagEnt, Diagram, furtherIterationObj) {
    ...
    if (!PgmAdded) {
        NewDiagInfo = CheckIfPgmInAnExistingClter(NewDiagEnt, Diagram, furtherIterationObj)
        Diagram = NewDiagInfo[0]
        furtherIterationObj = NewDiagInfo[1]
        PgmAdded = NewDiagInfo[2]
    }
