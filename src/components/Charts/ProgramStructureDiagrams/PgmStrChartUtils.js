...
export const findEndPointRelCoords = (EndPointRels, newEndPointCoords, descendants) => {
...
export const getPgmCoords = (DiagramData, searchValue) => {
  ...
  if (DiagramData.name === searchValue) return {centerX:DiagramData.x , centerY:DiagramData.y}
  
  const findPgmOnChildren  = (childrenArray,searchValue) => {
    if (childrenArray.length === 0) return; 
    console.log(childrenArray);
    childrenArray.forEach((child) => {
      if (child.data.name === searchValue) return {centerX:child.x, centerY:child.y}
      else {
        if (child.data.children.length > 0) findPgmOnChildren(child.children,searchValue)
      }
    });
  }
...
export const createChildLinks = (coords) => {
  ...
  return coords.map((coord) => {
    // for each parent 
    if (childAdjustment[coord.child.name]) childAdjustment[coord.child.name]+=1;
